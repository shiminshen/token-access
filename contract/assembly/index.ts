...
export function inc_allowance(
  ...
  if (u128.from(balance) > u128.Zero) {
    allowanceRegistry.set(
      keyFrom(owner_id, escrow_account_id),
      u128.from(u128.add(u128.from(balance), u128.from(amount)).lo)
    )
  } else {
...
export function dec_allowance(
  ...
  if (u128.from(balance) > u128.Zero) {
    assert(
      u128.sub(u128.from(balance), u128.from(amount)) > u128.Zero,
      ERR_DECREMENT_LESS_THAN_ZERO
    )
    allowanceRegistry.set(
      keyFrom(owner_id, escrow_account_id),
      u128.from(u128.sub(u128.from(balance), u128.from(amount)).lo)
    )
  } else {
    allowanceRegistry.delete(keyFrom(owner_id, escrow_account_id))
  }
